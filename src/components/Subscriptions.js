...
function Subscriptions(props) {
  const { onViewProfile } = props;
  const { subscriptions, currency, paymentSchedule, onUnsubscribe } = props;
  const { classes } = props;
        ...
        <TableCell className={classes.tableCell} numeric={true}>
          <Tooltip title={subscription.amount+' '+currency+' per month'}>
            <div className={classes.amount}>
              <Typography variant="subheading">
                {strings.currency[currency] + subscription.amount}
              </Typography>
            </div>
          </Tooltip>
        </TableCell>
        <TableCell className={classes.tableCell}>
          {!subscription.permanent && <Tooltip title="Unsubscribe">
            <Button className={classes.button}
              onClick={e => onUnsubscribe(subscription.hostname)}
